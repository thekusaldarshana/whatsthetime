...
export default function MeetingPlanner() {
  ...
  // useState section
   ...
   const [meetingDesc, setMeetingTitleDesc] = useState(''); // Corrected from setMeetingDesc
   ...
   const fromZone = params.get('from');
   const toZone = params.get('to');

   if (fromZone || toZone) {
      setParticipants((prev) => {
         const updated = [...prev];
         if (fromZone && isValidZone(fromZone)) updated[0].zone = fromZone;
         if (toZone && isValidZone(toZone)) updated[1].zone = toZone;
         return updated;
      });
      ...
      setPopupMessages((msgs) => ({
         ...msgs,
         autoToast: '🎉 Time zones loaded from your conversion!',
      }));

      setTimeout(() => {
         setPopupMessages((msgs) => {
            const copy = { ...msgs };
            delete copy.autoToast;
            return copy;
         });
      }, 5000);
   }
   ...
   const getEmojiForTime = (dt) => {
   const hour = dt.hour;
   const weekday = dt.weekday; // 1 = Monday ... 7 = Sunday

   if (weekday === 6 || weekday === 7) return { emoji: '⛱️', label: 'Weekend' };
   if (hour >= 0 && hour < 6) return { emoji: '🌙', label: 'Asleep (Late Night)' };
   if (hour >= 6 && hour < 9) return { emoji: '🌤️', label: 'Waking Up / Morning' };
   if (hour >= 9 && hour < 17) return { emoji: '💼', label: 'Typical Working Hours' };
   if (hour >= 17 && hour < 21) return { emoji: '🍽️', label: 'Evening / Dinner Time' };
   if (hour >= 21) return { emoji: '🌃', label: 'Night' };
...
const handleExportGoogleCal = () => {
         ...
         <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between gap-6 items-start w-full">
               <div className="flex-1 space-y-4 w-full">
                  ...
                  <button className="bg-yellow-500 hover:bg-yellow-600 text-black/80 dark:bg-green-500 hover:dark:bg-green-600 py-3 px-4 rounded-lg font-semibold" onClick={addParticipant}>➕ Add Participant</button>
               ...
               <details className="w-full md:w-64 mt-6 md:mt-0 bg-zinc-800 dark:bg-black/30 border border-yellow-300 dark:border-teal-700 rounded-lg p-4 text-sm text-gray-100 dark:text-gray-100">
                  <summary className="cursor-pointer font-semibold mb-2">Why These Colours & Emojis?</summary>
                  ...
                  <ul className="mt-2 space-y-1">
                     <li className= "mt-3 font-medium text-md">▼ Emojis</li>
                     <li>🌤️ Waking Up / Morning</li>
                     <li>💼 Typical Working Hours</li>
                     <li>🍽️ Evening / Dinner Time</li>
                     <li>🌃 Night</li>
                     <li>🌙 Asleep (Late Night)</li>
                     <li>⛱️ Weekend</li>
                  </ul>
               </details>
            </div>

         <div className="overflow-x-auto mt-6 border border-gray-300 dark:border-gray-600 rounded-xl table-scroll-container">
            <table className="min-w-full w-full text-sm text-left"> {/* Removed table-fixed */}
               <thead>
                  <tr className="bg-gray-100 dark:bg-white/10 text-black dark:text-white font-semibold">
                     <th className="p-3 w-16">Select</th> {/* Fixed width for "Select" column */}
                     <th className="p-3 w-28 min-w-[100px]">Time (UTC)</th> {/* Fixed width for "Time (UTC)" */}
                     {participants.map((p, idx) => {
                        const city = p.zone?.includes('/') ? p.zone.split('/')[1].replaceAll('_', ' ') : p.zone || 'Unknown';
                        return <th key={idx} className="p-3 min-w-[150px]">{`${p.name || 'Unknown'} (${capitalize(city)})`}</th>; {/* Added min-w for participant columns */}
                     })}
                  </tr>
               </thead>
               <tbody>
   {dates.map(date =>
      Array.from({ length: (24 * 60) / interval }).map((_, i) => {
         ...
         return (
            ...
            >
               <td className="p-2 text-center w-16"> {/* Fixed width for "Select" column */}
                  <input
                     type="checkbox"
               ...
               <td className="p-2 font-medium text-gray-700 dark:text-white/80 relative w-28 min-w-[100px]"> {/* Fixed width for "Time (UTC)" */}
                  {base.toFormat('HH:mm')}
                  {isCurrentSlot && (
               ...
               {participants.map((p, idx) => {
                  ...
                  const { emoji, label } = local.isValid ? getEmojiForTime(local) : { emoji: '', label: '' };

                  return (
                     <td key={idx} className="p-4 text-gray-600 dark:text-white/70 relative min-w-[150px]"> {/* Min width for participant time cells */}
                        {display} {emoji && <span className="ml-1" title={label}>{emoji}</span>}
                     </td>
                  );
               })}
            </tr>
   ...
   <div className="relative group">
      ...
      >
         ...
         {popupMessages.autoToast && (
   <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-purple-300 dark:bg-green-300 text-black/90 px-6 py-4 rounded-xl shadow-lg z-50 text-sm font-medium animate-fade-in-out">
      {popupMessages.autoToast}
   </div>
)}
         ...
         {showSchedulePopup && (
            <div className="sticky flex items-center justify-center px-4">
               <div className="bg-white dark:bg-[#000] p-6 rounded-xl w-full max-w-2xl shadow-xl relative">
                  ...
                  <form className="space-y-6">
                     ...
                     <textarea
                           ...
                           value={meetingDesc}
                           onChange={(e) => setMeetingTitleDesc(e.target.value)} // Corrected from setMeetingDesc
   ...
   {baseTime.toFormat('ccc, LLL dd')} – {baseTime.toFormat('HH:mm')} UTC
         ...
         <style>{`
            ...
            .table-scroll-container::-webkit-scrollbar {
              width: 10px; /* Adjust for desired thickness */
            ...
            .table-scroll-container::-webkit-scrollbar-thumb {
              background: #888; /* Darker color for the scroll thumb */
              border-radius: 10px;
            }
            ...
            /* Dark mode considerations (example - you'd likely integrate with your Tailwind dark mode setup) */
            ...
            .dark .table-scroll-container {
              scrollbar-color: #bbb #333; /* thumb color track color for dark mode */
            }
